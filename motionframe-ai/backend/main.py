...
from ai.animatediff import AnimateDiff, generate_video_from_image

animatediff = AnimateDiff()

@app.on_event("startup")
async def startup_event():
    animatediff.load_models()
...
async def generate_video_sse(
    ...
    try:
        logger.info(f"Received request with image_filename: {image_filename}, prompt: {prompt}, duration: {duration}")
        image_path = os.path.join("../storage/uploads", image_filename)
        ...
        logger.error(f"Error in generate_video_sse: {e}")
        raise HTTPException(status_code=500, detail=str(e))
